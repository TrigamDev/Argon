...
function useOutsideAlerter(ref, clickEvent) {
	useEffect(() => {
		function handleClickOutside(event) {
			if (ref.current && !ref.current.contains(event.target)) clickEvent()
		}
...
export class Multiselect extends React.Component<IMultiselectProps, any> {
	...
	removeSelectedValuesFromOptions(skipCheck) {
		const { isObject, displayValue, groupBy, areOptionsEqual } = this.props
		...
		if (isObject) {
			let optionList = unfilteredOptions.filter(item => {
				return selectedValues.findIndex(
					v => {
						if ( areOptionsEqual ) return areOptionsEqual(v, item)
						else return v[displayValue] === item[displayValue]
					}
				) === -1 ? true : false
			})
			if (groupBy) {
	...
	onRemoveSelectedItem(item) {
		...
		const { onRemove, showCheckbox, displayValue, isObject, areOptionsEqual } = this.props
		if (isObject) {
			index = selectedValues.findIndex(
				i => {
					if ( areOptionsEqual ) return areOptionsEqual(i, item)
					else return i[displayValue] === item[displayValue]
				}
			)
		} else {
	...
	isSelectedValue(item) {
		const { isObject, displayValue, areOptionsEqual } = this.props
		const { selectedValues } = this.state
		if (isObject) {
			return (
				selectedValues.filter(i => {
					if ( areOptionsEqual ) return areOptionsEqual(i, item)
					else return i[displayValue] === item[displayValue]
				}).length > 0
			)
	...
	isOptionIncluded ( option: any ) {
		...
		if (isObject) {
			return (
				options.filter(i => {
					if ( areOptionsEqual ) return areOptionsEqual(i, option)
					else return i[displayValue] === option[displayValue]
				}).length > 0
			)
		}
		return options.filter(i => i === option).length > 0
	}
	...
	renderOptionList() {
		...
		let evaluated = this.props.evaluateValue
			? this.props.evaluateValue( inputValue )
			: null
		if (evaluated && !this.isOptionIncluded( evaluated )) options.push( evaluated )
		
		return (
			<ul className={`optionContainer`} style={style['optionContainer']}>
				{ options.length === 0 &&
					<span style={style['notFound']} className={`notFound`}>{emptyRecordMsg}</span>
				}
	...
	isDisablePreSelectedValues(value) {
		const { isObject, disablePreSelectedValues, displayValue, areOptionsEqual } = this.props
		...
		if (isObject) {
			return (
				preSelectedValues.filter(
					i => {
						if ( areOptionsEqual ) return areOptionsEqual(i, value)
						else return i[displayValue] === value[displayValue]
					}
				).length > 0
			)
		}
