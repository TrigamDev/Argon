...
export default function SettingsHamburger({ updateSettings }: { updateSettings: CallableFunction }) {
...
function ToggleOption({ label, current, onChange }: { label: string, current: boolean, onChange: CallableFunction } ) {
    ...
    return (
        <div className="hamburger-item toggle-option" onClick={flipToggle}>
...
function DropDownOption({ label, options, current, onChange }: { label: string, options: any, current: string, onChange: (arg: Option) => void } ) {
    ...
    return (
        <div className="hamburger-item dropdown-option" onClick={(e) => { e.stopPropagation() }}>
            <span className="label">{label}</span>
            <Dropdown
